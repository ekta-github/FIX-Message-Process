 ...
 * 
 * This file is provided AS IS with NO WARRANTY OF ANY KIND, INCLUDING z
...
public class MessageUtils1 {
    ...
    int beginStringLen = dataDictionary.getVersion().length();
		String beginString = messageString.substring(2, 2 + beginStringLen);//todo:FIX.?.? or STEP.?.?.?'\
		System.out.print("Messageeeee *******" + messageString);
		String messageType = getMessageType(messageString);
		quickfix.fix44.Message message = (quickfix.fix44.Message) messageFactory.create(beginString,
				messageType);
